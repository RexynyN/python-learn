...
def get_images(src, shift):
    ...
    for i in shift:
        ...
        request_image(image_url, img_path + filename)
...
def request_image(image_url, img_path):
    while True:
        try:
            ...
            image_url = inquirer.text(f"Não conseguimos baixar {image_url}. Verifique a URL ou digite \"q\" para pular a imagem")
        else:
            break 
...
def img_to_pdf(files, path, name, keep=False):
...
def img_to_epub(files, path, name):
    print("Criando arquivo...")
    book = epub.EpubBook()
...
def main(action):
    if(action == "folder"):
        dir = inquirer.text(message="Digite o diretório das imagens (a ordem do nome das imagens será mantido)")
        name_file = inquirer.text(message="Digite o nome do arquivo final (ENTER para padrão)", default="")
        ...
        img_to_pdf(dir_files, dir, f"{name_file}.pdf")

    elif(action == "dataurl"):
        dir = inquirer.text(message="Digite o caminho com .json com as dataurls")
        name_file = inquirer.text(message="Digite o nome do arquivo final (ENTER para padrão)", default="")
        
        try:
            path = dataurl_to_image(dir)
        except IndexError:
            print_help()
            exit()
        ...
        img_to_pdf(dir_files, path, f"{name_file}.pdf")

    elif(action == "web"):
        url = inquirer.text(message="Digite a url (coloque ?? onde tiver que substituir a variação)")

        shift = inquirer.text(message="Coloque o que substituir (um número de 1 a N ou uma lista de valores separados por vírgula)")
        ...
        img_to_pdf(dir_files, path, f"{name_file}.pdf", keep)
        ...
        img_to_epub(dir_files, dir, f"{name_file}.epub")
    else:
        print_help()
